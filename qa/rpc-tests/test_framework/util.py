...
def initialize_datadir(dirname, n):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "absolute.conf"), 'w') as f:
        f.write("regtest=1\n")
        f.write("rpcuser=rt\n")
...
def wait_for_bitcoind_start(process, url, i):
    '''
    Wait for absoluted to start. This means that RPC is accessible and fully initialized.
    Raise an exception if absoluted exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('absoluted exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir):
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i) # Overwrite port/rpcport in absolute.conf
...
def connect_nodes(from_connection, node_num):
    ...
    # poll until version handshake complete to avoid absolute conditions
