...
*Setup instructions for a Gitian build of Absolute Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Absolute
...
- [Building Absolute Core](#building-absolute-core)
...
git clone https://github.com/absolutecrypto/absolute
```
...
in the Absolute Core repository under 'Fetch and build inputs' to install sources which require
...
```bash
    ...
    From https://github.com/absolutecrypto/absolute
...
and the absolute git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../absolute/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/absolutecrypto/absolute-detached-sigs
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/absolute-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/absolute-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/absolute-osx-build.assert
```
