...
const char * const BITCOIN_CONF_FILENAME = "absolute.conf";
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AbsoluteCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AbsoluteCore
    // Mac: ~/Library/Application Support/AbsoluteCore
    // Unix: ~/.absolutecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AbsoluteCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AbsoluteCore";
#else
    // Unix
    return pathRet / ".absolutecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty absolute.conf if it does not excist
